...
class DatapathPTWIO(implicit p: Parameters) extends CoreBundle()(p)
  ...
  val hgatp = Input(new PTBR())
  val vsatp = Input(new PTBR())
  /** menvcfg.PBMTE: Svpbmt enable for satp/hgatp-controlled translation */
  val pbmte = Input(Bool())
  /** effective henvcfg.PBMTE: Svpbmt enable for VS-stage translation */
  val hpbmte = Input(Bool())
...
/** Svpbmt page-based memory type field values (PTE[62:61])
  * @see RV-priv "Svpbmt" extension
  */
object PBMT {
  def PMA  = 0 // None: use the PMA-defined memory type
  def NC   = 1 // Non-cacheable, idempotent, weakly-ordered main memory
  def IO   = 2 // Non-cacheable, non-idempotent, strongly-ordered I/O
  def RSVD = 3 // Reserved
}
...
class PTE(implicit p: Parameters) extends CoreBundle()(p) {
  /** Svnapot bit (PTE[63]); unsupported here, so it must be zero */
  val n = Bool()
  /** Svpbmt page-based memory type (PTE[62:61]); see [[PBMT]] */
  val pbmt = UInt(2.W)
  val reserved_for_future = UInt(7.W)
  val ppn = UInt(44.W)
  ...
  /** return true if find a pointer to next level page table.
    * A non-leaf PTE must have all reserved fields -- including the Svnapot (N)
    * and Svpbmt (PBMT) bits -- zeroed, regardless of whether those extensions
    * are enabled. */
  def table(dummy: Int = 0) = v && !r && !w && !x && !d && !a && !u && !n && pbmt === 0.U && reserved_for_future === 0.U
...
class L2TLBEntry(nSets: Int)(implicit p: Parameters) extends CoreBundle()(p)
  ...
  /** whether the page is readable */
  val r = Bool()
  /** Svpbmt page-based memory type (only stored when Svpbmt is enabled) */
  val pbmt = usingSvpbmt.option(UInt(2.W))
...
class PTW(n: Int)(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  ...
  val (l2_hit, l2_error, l2_pte, l2_tlb_ram) = if (coreParams.nL2TLBEntries == 0) (false.B, false.B, WireDefault(0.U.asTypeOf(new PTE)), None) else {
    ...
    when (l2_refill && !invalidated) {
      ...
      entry.w := r_pte.w
      entry.r := r_pte.r
      entry.pbmt.foreach(_ := r_pte.pbmt)
      entry.tag := r_tag
    ...
    s2_pte.pbmt := s2_hit_entry.pbmt.getOrElse(0.U)
    s2_pte.n := false.B
    s2_pte.v := true.B
  ...
  val merged_pte = {
    ...
    val res = makePTE(stage1_ppn & superpage_mask, aux_pte)
    if (usingSvpbmt) {
      // Two-stage translation combines the VS-stage (aux_pte) and G-stage (pte)
      // PBMT fields.  The VS-stage (guest) type takes precedence when it is not
      // PMA (nonzero); otherwise the G-stage (host) type applies.  The TLB
      // additionally folds in the PMA, so this can never relax a genuinely
      // effectful/uncacheable region -- it can only downgrade cacheable main
      // memory to NC/IO.
      res.pbmt := Mux(aux_pte.pbmt =/= 0.U, aux_pte.pbmt, pte.pbmt)
    }
    res
  }
  ...
  when (mem_resp_valid) {
    ...
    when (traverse) {
      ...
      // Svpbmt: a nonzero PBMT (bits [62:61]) is legal only on a leaf PTE, only
      // for the NC/IO encodings (not the reserved value 3), and only when the
      // enable bit for this translation stage is set -- menvcfg.PBMTE for the
      // S-stage/G-stage walks, henvcfg.PBMTE for the VS-stage (guest) walk.
      // The middle reserved bits must always be zero.  The N bit (Svnapot) is
      // legal only on a last-level (4 KiB) leaf whose ppn[3:0] encodes the one
      // defined NAPOT size, 64 KiB (ppn[3:0] == 0b1000); otherwise it faults.
      // Anything else is a page fault, exactly as before when those ten bits
      // were one undifferentiated reserved field.
      val cur_pbmte = Mux(do_both_stages && !stage2, io.dpath.hpbmte, io.dpath.pbmte)
      val pbmt_legal = pte.pbmt === 0.U ||
        (usingSvpbmt.B && cur_pbmte && pte.pbmt =/= PBMT.RSVD.U && pte.leaf())
      val n_legal = !pte.n ||
        (usingSvnapot.B && pte.leaf() && count === (pgLevels-1).U && pte.ppn(3,0) === "b1000".U)
      val res_ok = n_legal && pte.reserved_for_future === 0.U && pbmt_legal
      val gf = (stage2 && !stage2_final && !pte.ur()) ||
        (Mux(count === (pgLevels - 1).U, pte.leaf(), pte.v) && res_ok && invalid_gpa)
